...
class JoyToJoint4DoF(Node):
    def __init__(self):
        ...
        qos_js = QoSProfile(depth=10, reliability=ReliabilityPolicy.RELIABLE, durability=DurabilityPolicy.TRANSIENT_LOCAL) # RELIABLE=1, TRANSIENT_LOCAL=2
        self.create_subscription(JointState, '/joint_states', self.js_cb, qos_js)
    ...
    def control_loop(self):
        ...
        if not deadman_active:
            # Continuously sync internal target to real-time Unity state
            cur = dict(zip(self.latest_joint_state.name, self.latest_joint_state.position))
            if all(j in cur for j in ARM_JOINTS):
                self.q_target = np.array([cur[j] for j in ARM_JOINTS])
            return  # IMPORTANT: Do not publish when idle!
        else:
