...
public class RobotStateReceiver : MonoBehaviour
{
    ...
    [Tooltip("ROS topic carrying the Unity twin's commanded joint positions")]
    public string stateTopic = "/servo_joint_target";
    ...
    private Dictionary<string, float> _currentPositionsRad = new Dictionary<string, float>();

    public float GetPositionRad(string jointName)
    {
        if (_currentPositionsRad.TryGetValue(jointName, out float val)) return val;
        return 0f;
    }
    ...
    private float _ignoreIncomingUntil = 0f;

    public void IgnoreIncoming(float seconds)
    {
        _ignoreIncomingUntil = Time.time + seconds;
    }
    ...
    IEnumerator SubscribeGeneric()
    {
        ...
        Debug.Log($"<color=yellow>[RobotStateReceiver] Subscribing to {stateTopic}...</color>");
        RosConnector.Instance.Subscribe(stateTopic, "sensor_msgs/msg/JointState", HandleMessage);
    }
    ...
    void HandleMessage(string jsonString)
    {
        ...
        try
        {
                ...
                && packet.msg.name.Length > 0 && packet.msg.position.Length > 0)
            {
                ...
                if (Time.time > _ignoreIncomingUntil)
                {
    ...
    public void UpdateJoint(string jointName, float posRad)
    {
        if (_jointMap.ContainsKey(jointName))
        {
            _currentPositionsRad[jointName] = posRad;
            try
            {
